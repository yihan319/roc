      ...
      <div className="fixed top-0 left-0 w-full shadow-lg p-2 z-50" style={{ backgroundColor: "#8EB9CC" }}>
        <div className="flex justify-between items-center">
          <ul className="menu flex gap-4 text-xl lg:text-3xl">
            <li>
              <Link href="/dashboard" className="font-bold" style={{ color: "#2C3E50" }}>
                首頁
              </Link>
            </li>
            <li>
              <Link href="/pages/reports" className="font-bold" style={{ color: "#2C3E50" }}>
                通報
              </Link>
            </li>
            <li>
              <a href="https://fhy.wra.gov.tw/fhyv2/alert/warn" className="font-bold" style={{ color: "#2C3E50" }}>
                水情資訊
              </a>
            </li>
            <li>
              <a href="https://www.cwa.gov.tw/V8/C/" className="font-bold" style={{ color: "#2C3E50" }}>
                天氣狀況
              </a>
            </li>
            <li>
              <Link href="/map" className="font-bold" style={{ color: "#2C3E50" }}>
                地圖路徑
              </Link>
            </li>
            {user?.role === "admin" && (
              <li>
                <Link href="/pages/admin-case" className="font-bold" style={{ color: "#2C3E50" }}>
                  案件審核
                </Link>
              </li>
            )}
            {user?.role === "volunteer" && (
              <li>
                <Link href="/pages/volunteer-case" className="font-bold" style={{ color: "#2C3E50" }}>
                  志工接案
                </Link>
              </li>
            )}
            {user?.role === "member" && (
              <li>
                <Link href="/pages/member-case" className="font-bold" style={{ color: "#2C3E50" }}>
                  我的案件
                </Link>
              </li>
            )}
            {user?.role === "admin" && (
              <li>
                <Link href="/pages/licenses" className="font-bold" style={{ color: "#2C3E50" }}>
                  證照認證
                </Link>
              </li>
            )}
          ...
          <ul className="menu flex gap-4 text-xl lg:text-3xl">
            {user ? (
              <li className="font-bold" style={{ color: "#2C3E50" }}>
                歡迎, {user.name}
              </li>
            ) : (
              <>
                <li>
                  <Link href="/pages/signup" className="font-bold" style={{ color: "#2C3E50" }}>
                    註冊
                  </Link>
                </li>
                <li>
                  <Link href="/pages/signin" className="font-bold" style={{ color: "#2C3E50" }}>
                    登入
                  </Link>
                </li>
              </>
            ...
            <li>
              <Link href="/" className="font-bold" style={{ color: "#8f0202ff" }}>
                登出
              </Link>
            </li>
          </ul>
